...
timeReductionStr='\tSlicing time data using strategy: %s';
...
if (timeReduction > 1 && timeReduction < timeSteps)
    ...
    display(sprintf('\toriginal steps: %i, sliced steps: %i',timeSteps,timeReduction));
elseif (timeReduction <= 1 && timeReduction > 0)
    ...
    display([ sprintf(timeReductionStr,'fraction of total number of steps') ' ' num2str(timeReduction)] );
    display(sprintf('\toriginal steps: %i, sliced steps: %i',timeSteps,newSteps));
else
