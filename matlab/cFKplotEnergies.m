function cFKplotEnergies( ts, E_Ks, hE, numSolitons, cFKaxes, cFKi, cFKsimParams, runList, labelFigs )
    ...
    axes(cFKaxes.spring);
    hold on;
    smoothKE = min(325,round(length(E_Ks(:,2))/2));
    [ kEsmooth, ~ ] = savitzyGolaySmoothing( 2, smoothKE, E_Ks(:,2), 1 );
    cFKplot(ts*1e9, kEsmooth, cFKi, 1, '-', '', '', 'Spring (K)', {['smoothed <' smoothKE '>']}, labelFigs);
    axes(cFKaxes.substrate);
    hold on;
    % cFKplot(ts*1e9, E_Ks(:,3), cFKi, 1, '-', '', '', 'Substrate (K)', {}, labelFigs);
    [ hEsmooth, ~ ] = savitzyGolaySmoothing( 2, 25, sum(hE,2), 1 );
    % hEsmooth = sum(hE,2);
    cFKplot(ts*1e9, hEsmooth, cFKi, 1, '-', '', '', 'Substrate (K)', {'smoothed <25>'}, labelFigs);
    axes(cFKaxes.total);
    hold on;
