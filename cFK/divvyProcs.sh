...
Nlist=($Nlist); export Nlist
cooltimes="1e6 2e6"; export cooltimes
coolList=($cooltimes); export coolList
startTemps=$( seq 300 100 400 ); export startTemps
startTlist=($startTemps); export startTlist
enslist=$( seq 1 1 2 ); export enslist
...
Flist="0 1e-17"; export Flist
Flist=($Flist);
./sweep.pl
for eqti in ${coolList[@]}
do
...
echo "eqt:$eqti"
echo "Tst:$Ti"
    cFKdataChanges=$(( ${#coolList[@]} * ${#startTlist[@]} * ${#Nlist[@]} ))
    ...
    sed "s/Tstart=0/Tstart=$Ti/" <cFKdata.xy.eqt.f90 >cFKdata.xy.forquest.f90
    cFKchanges=${#Flist[@]}
    ...
    if [ 1 ]; then #"$cFKchanges" -gt "1" -o "$cFKdataChanges" -gt "1" ]; then
        ...
        cp ifort.forquest.out questbatches/ifort.eq${eqti}.Tst${Ti}.N${Ni}.ens$ensi.k${ki}.F${Fi}.out
    fi
    ...
    sed "s/ifort.out/ifort.eq${eqti}.Tst${Ti}.N${Ni}.ens$ensi.k${ki}.F${Fi}.out/" <questbatch.sh >questbatches/runeq${eqti}.Tst${Ti}.N${Ni}.ens${ensi}.k${ki}.F${Fi}.sh
    #exe msub -N eq${eqti}Tst${Ti}N${Ni}E${ensi}k${ki}F${Fi} -l procs=1,walltime=00:05:00 -joe -V -o seeout.log questbatches/runeq${eqti}.Tst${Ti}.N${Ni}.ens${ensi}.k${ki}.F${Fi}.sh
done
done
done
done
