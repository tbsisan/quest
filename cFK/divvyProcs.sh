...
make_cFKdata_precompile() {
    ...
    sed "s/kTrap=0/kTrap=$kTrapi/" <cFKdata.xy.T.f90 >cFKdata.xy.batch.f90
}
...
do
    myrand=$(getRandomInt);
    echo "$kTrapi, $eqti, $Ti, $Ni, $ensi, $ki, $Fi, $myrand";
    customRun=r${myrand}.kTr${kTrapi}.eq${eqti}.Tst${Ti}.N${Ni}.ens$ensi.k${ki}.F${Fi}
    #INTEGER, PARAMETER :: N=208, Nsim=208,  channelWL=200
    cFKdataChanges=$(( ${#eqList[@]} * ${#startTlist[@]} * ${#Nlist[@]} * ${#kTrapList[@]} ))
    cFKchanges=${#Flist[@]}
