...
!                                                                      C
!CCCCCCCCCCCCCCCCC1CCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCC
!
private
public :: rannw,ranut
CONTAINS
...
!   parallel computers", Proc. Fourth International Euro-Par !onference,
!   Lecture Notes in !omputer Science, Vol. 1470, Springer-Verlag, 
!   Berlin, 1998, 1-20.
!
...
!
        integer j, jlim, jlow, flag, lbp1, lbp2, lbpf, nloc, nd, usiz
        integer, save :: f=4
...
! f should be in the range 1 to 8 (recommended value is 2, 3, or 4).
        real r, t, pi2, ssq, scale
        integer nbuf, uind, x1ind, y1ind, x2ind, y2ind, z2ind
        integer, save :: nz2=63
! nz2 should be at least 58 and no larger than nbuf
!
! TBS: change errort from 1d-6 to 1d-3
        real, save :: checkt=1.0D-3, errort=1.0D-3
...
!
          nbuf = 64
100        nbuf = 2*nbuf
          if (nbuf .le. (nwork-(nz2+7))/10) go to 100
         print*,'Nbuf = ',nbuf
! !heck for nwork too small
          if (nbuf .lt. 256) then
...
! Following loop vectorises
          ...
          ssq = ssq + t
          r = sqrt(t)
          dwork(x1ind+j) = r*sin(pi2*dwork(y2ind+j))
          dwork(y1ind+j) = r*cos(pi2*dwork(y2ind+j))
        enddo
...
! (it has the correct mean nu and variance 2*nu).
          dwork(5) = sqrt(ssq)
          dwork(6) = sqrt(1.0d0/(1.0d0 + &
                      sqrt(1.0d0 - 1.0d0/dfloat(lbp2))))
          dwork(7) = sqrt(sqrt(dfloat(lbp2)*dfloat(lbp2-1)))
        endif
! Usual entry - restore variables saved in dwork
        ...
        lbp2 = int(dwork(3))
        flag = int(dwork(4))
! !heck consistency of dwork(1) to dwork(7)
...
! by rounding errors so probably caused by overwriting buffer.
            if (abs(ssq - dwork(5)) .ge. errort*ssq) then
...
! Reverse flag (alternates between 0 and lbp2 = 2*nbuf)
        ...
        go to 300
        end subroutine rannw
!
        subroutine rannwb (n, scale, x1, y1, x2, y2, z2, nz2)
!       implicit none
        integer n, nz2
        real scale, x1(*), y1(*), x2(*), y2(*), z2(*)
!
...
!
        ...
        real t1, t2, a00, a01, t, tr
        real c0, c1
        c0 = 1.0d0/(2.0d0 + sqrt(3.0d0))
        c1 = 1.0d0/sqrt(3.0d0) - c0
        k3 = nz2
...
! gamma and delta should be nonnegative.
        ...
        j0 = 0
! while (j0 .lt. n) ...
! !ompute j1, k1, k2 so no "mod n" operations on indices in loop
! (so conceptual loop is split into about alpha + beta smaller loops)
1000      k1 = gamma + 1 - n*((alpha*j0 + gamma)/n)
...
!
        j0 = j1 + 1
        if (j0 .lt. n) go to 1000
        return
        end subroutine rannwb
! End of rannw.f
...
! xb could be logical but we have made it real
...
! (xb could be an array of characters or bits - real is
